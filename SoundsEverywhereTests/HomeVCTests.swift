...
class HomeVCTests: XCTestCase {
    ...
    func testInitialCollectionViewState() throws {
        let vc = try makeHomeVC()
        vc.loadViewIfNeeded()
        XCTAssertEqual(vc.collectionView.numberOfSections, 0)
    }
...
//    }
    ...
    private func makeHomeVC() throws -> HomeVC {
        ...
        let navVC = try XCTUnwrap(tabBar.viewControllers?.first as? UINavigationController)
        let mockVC = try XCTUnwrap(navVC.topViewController as? HomeVC)
